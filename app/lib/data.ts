...
export async function fetchLatestInvoices() {
  try {
    const data = await sql<LatestInvoiceRaw>`
      ...
      ORDER BY invoices.date DESC, invoices.time DESC
      LIMIT 5`
...
export async function fetchCardData() {
  try {
    ...
    const dataRow = sql`SELECT
         SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END) AS paid,
         SUM(CASE WHEN status = 'pending' THEN amount ELSE 0 END) AS pending,
         COUNT(*) AS count1,(SELECT COUNT(*) AS count2 FROM customers) FROM invoices `

    const data = await dataRow
    const numberOfInvoices = Number(data.rows[0].count1 ?? '0')
    const numberOfCustomers = Number(data.rows[0].count2 ?? '0')
